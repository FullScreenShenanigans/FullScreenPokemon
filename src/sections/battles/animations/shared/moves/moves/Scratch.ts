...
export class Scratch extends Move {
    ...
    public runAnimation(callback: () => void): void {
        ...
        this.game.things.add(scratches[1], startX + offset * this.direction * -1, startY + offset);
        this.game.things.add(scratches[2], startX + offset * this.direction * -2, startY + offset * 2);
            ...
            (): void => {
                for (const scratch of scratches) {
                    ...
                    this.game.timeHandler.addEvent(
                        (): void => this.game.physics.shiftHoriz(scratch, differenceX * this.direction / 16),
                        1);
                    this.game.timeHandler.addEvent(
                        (): void => this.game.physics.shiftVert(scratch, differenceX * this.direction / 16),
                        1);
