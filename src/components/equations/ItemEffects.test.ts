...
describe("ItemEffects", () => {
    ...
    it("adds a new Pokemon to your PC when your party has more than six Pokemon", (): void => {
        // Arrange
        const { fsp } = stubBlankGame();
        ...
        expect(fsp.itemsHolder.getItem(fsp.storage.names.pokemonInPC)[0].title).to.be.equal(charmander.title);
    });
});
